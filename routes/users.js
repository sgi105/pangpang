...
exports.login = async (req, res) => {
  ...
  try {
    ...
    user_token = await token.generateToken(
...
exports.autoLoginCheck = async (req, res) => {
  ...
  try {
    ...
    //50이면 홈 화면으로 이동
    const { user_token } = req.headers;
    const decodedTokenResult = await token.decodedToken(
      user_token,
      req.app.get("jwt-secret")
    );
    userDataResult = await userData.getMypageUserInfo(
      decodedTokenResult.userID,
      pool
    );
